...
show_help() {
    ...
    echo
    echo "COMPACT DATABASE UPLOAD: Uploads list of raw files + metadata from the CSV and pipeline settings from Nextflow configs to the database. WARNING: Ensure 'contra.config' and 'nextflow.config' are set before running."
    echo
    echo "Arguments:"
...
if [[ "$(hostname)" != "csched" ]]; then
    echo "Error: This script must be run on the 'csched' machine."
    exit 1
fi
...
apptainer_image="/b/u/vishnu/SINGULARITY_IMAGES/skyweaverpy_latest.sif"
mounts="-B /homes -B /b -B /bscratch"
...
set_up_cfg_file="nextflow config -profile contra -flat -sort > nf_config_for_data_upload.cfg"
